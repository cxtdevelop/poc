...
if routeID in ['TXR1', 'TXR2', 'DTR1']:
  ...
  send(local_url, "TX_REGISTER", "0", "0")
  send(control_url, "TX_REGISTER", "0", "0")
...
#          logging.warning("Destination = %s " % (TayoDestination))
      ...
      if deviceID[:2] in ['TX']: ############### TX TX TX TX TX TX TX TX
        #getPositionData(gpsd)
        nx = gpsd.next()
        if nx['class'] == 'TPV':
          ...
          send(local_url, "TX_LOG", "0", "0")
          send(control_url, "TX_LOG", "0", "0")
  ...
  try:
    while True:
      ...
      if rx_simpang == deviceID:
        ...
        if (tx_destination=='RAJAWALI_SELATAN') and (task == 'INS_DUELON'):
          logging.warning(">>>Order %s to LC %s %s destination %s  in phase 1" % (task, deviceID, deviceName, tx_destination))
          phase1.off()
        if (tx_destination=='RAJAWALI_TENGAH') and (task == 'INS_DUELOFF'):
          logging.warning(">>>Order %s to LC %s %s destination %s  in phase 1" % (task, deviceID, deviceName, tx_destination))
          phase1.on()

        if (tx_destination=='PURABAYA_TENGAH') and (task == 'INS_DUELON'):
          logging.warning(">>>Order %s to LC %s %s destination %s  in phase 2" % (task, deviceID, deviceName, tx_destination))
          phase2.off()
        if (tx_destination=='PURABAYA_SELATAN') and (task == 'INS_DUELOFF'):
          logging.warning(">>>Order %s to LC %s %s destination %s  in phase 2" % (task, deviceID, deviceName, tx_destination))
          phase2.on()

        #only on Simpang 40 Pandegiling
        if (tx_destination=='RAJAWALI_TENGAH') and (task == 'INS_DUELON'):
          logging.warning(">>>Order %s to LC %s %s destination %s in phase 1" % (task, deviceID, deviceName, tx_destination))
          phase1.off()
        if (tx_destination=='RAJAWALI_UTARA') and (task == 'INS_DUELOFF'):
          logging.warning(">>>Order %s to LC %s %s destination %s  in phase 1" % (task, deviceID, deviceName, tx_destination))
          phase1.on()
 
        if (tx_destination=='PURABAYA_UTARA') and (task == 'INS_DUELON'):
          logging.warning(">>>Order %s to LC %s %s destination %s  in phase 1" % (task, deviceID, deviceName, tx_destination))
          phase2.off()
        if (tx_destination=='PURABAYA_TENGAH') and (task == 'INS_DUELOFF'):
          logging.warning(">>>Order %s to LC %s %s destination %s  in phase 1" % (task, deviceID, deviceName, tx_destination))
          phase2.on()
